...
## Sub-domain Playbooks

The skills below were merged into this umbrella as references. They cover specialized sub-domains of "working with Hermes Agent" that don't fit in the CLI reference above but still belong in the same library. Load the specific reference when the trigger fires.

- [`references/setup-audit.md`](references/setup-audit.md) — Audit and optimize a Hermes install: the 4-command diagnostic combo (`doctor`, `status`, `config check`, `tools list`), browser tool gotchas (needs `agent-browser` + Playwright Chromium), web search backend config, smart approval mode. Load when the user says "check Hermes config", "optimize Hermes", "set up a new Hermes machine", or "why is tool X not working". The diagnostic companion script lives at `scripts/audit.sh`.
- [`references/skill-authoring.md`](references/skill-authoring.md) — Author SKILL.md files for the in-repo `skills/<category>/<name>/` tree. Validator constraints (≤1024 char description, required frontmatter fields), peer-matched structure, common pitfalls, directory placement. Load when the user asks to add a skill in this branch / repo / commit, or when editing an existing in-repo skill under `skills/`.
- [`references/s6-container-supervision.md`](references/s6-container-supervision.md) — Hermes Agent Docker image's s6-overlay supervision tree: `cont-init.d` boot scripts, `/run/service/gateway-<name>/` per-profile gateway slots, the Architecture B CMD-as-main-program pattern (and why s6-supervised main was abandoned). Load when modifying the Dockerfile, adding a new static service, debugging a profile gateway that won't start, or changing the per-profile gateway run command.
- [`references/tui-command-debugging.md`](references/tui-command-debugging.md) — Debug Hermes TUI slash commands across the Python registry / tui_gateway JSON-RPC bridge / Ink TypeScript frontend. Symptoms (missing autocomplete, CLI-vs-TUI drift, persisted config not applied live), investigation commands, and the canonical fix patterns. Load when a slash command shows in one layer but not another, or when TUI state doesn't reflect a recent config change.
