...
## Mobile + Audio Artifacts

For one-off HTML that plays audio (TTS readers, narrated decks, audio guides) and needs to be viewed on a phone/tablet over WiFi, see `references/mobile-html-with-audio.md`. It covers the iOS audio gesture gate, single-`<audio>` + src-swap pattern, mobile viewport + safe-area-inset CSS, swipe/tap-zone gestures, the `serve.py` LAN launcher, TOC thumbnail grid, and the file-layout pattern that works under both `file://` and `http://`.
...
- Do not autoplay audio without a user gesture on mobile Safari — it silently fails. Use the gate pattern from the mobile-audio reference.
