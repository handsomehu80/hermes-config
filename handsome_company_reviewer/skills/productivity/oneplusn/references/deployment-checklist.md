...
| 5b | **Invite each new employee as a collaborator on the org repo** | every employee can see the repo from its own PAT | boss runs `gh api /repos/<org>/<repo>/collaborators/<employee> -X PUT -f permission=push` per employee. **Without this, the employee's cron tick will silently poll nothing.** See `references/employee-repo-access.md` for symptom/detection/repair. |
...
8. ⚠️ **邀请步骤**: 老板你愿意现在就把每个员工 invite 为 `<org>/<repo>` 的 collaborator 吗?
   (必须做,否则员工 cron 会一直 404 — 详见 `references/employee-repo-access.md`)
```

If the user can't answer all of them at once, prioritize 1-5 (everything from there down the chain blocks). Step 6-7 can be deferred. Step 8 must be done before first cron tick — block the user on it if they say "later".
...
# 4b. ⚠️ Invite each employee as collaborator on the repo (else their cron 404s)
gh api /repos/<org>/agent_workflow/collaborators/<gh-user-1> -X PUT -f permission=push
gh api /repos/<org>/agent_workflow/collaborators/<gh-user-2> -X PUT -f permission=push
