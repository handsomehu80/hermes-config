...
**Parent-link + review-required trap.** If you chain cards with `--parent` (research → implement → review), and the implementer calls `kanban_block(review-required: ...)` to flag "shipped but needs eyes", the implementer card goes to `blocked` not `done`. The dispatcher's auto-promotion only fires on `done` parents, so the review card sits in `todo` forever. You'll see the implementer show ✓ done, the review stuck on ◻ todo, and the only way out is `hermes kanban unblock <implementer-id>`. Two ways to avoid this:
  1. **Tell implementers to use `kanban_complete` with a `kanban_comment` containing the "review-required" marker** — the handoff is in the comment thread, the card lands in `done`, the reviewer auto-promotes. This is the cleanest pattern.
  2. **If the implementer insists on `kanban_block` for review-required**, the orchestrator must `kanban unblock` it after the work is shipped but before the child can start. The unblock re-spawns the worker; if the work is already done, the worker re-reads the comment thread and immediately calls `kanban_complete` (cheap, ~150s overhead).

  Discovered the hard way on a 4-profile team build (2026-06-03): the eng's SOUL.md told it to use `kanban_block(review-required)`, which trapped the qa card until the PM manually unblocked. Document this convention in every implementer-profile's SOUL.md, not just rely on workers reading this skill.
