...
## P10. Profile's `home/` subdir is empty — real state lives at profile root

**Reproduction (in a fresh terminal inside a profile):**

```bash
$ ls -la $HOME
total 4
drwxr-xr-x 1 Administrator 197121 0  7月 12 20:03 .
drwxr-xr-x 1 Administrator 197121 0  7月 12 20:03 ..
# empty — where's the session data?

$ session_search
# 0 results — has this profile ever been used?
```

**Why:** Each Hermes profile lives at `~/AppData/Local/hermes/profiles/<name>/`. The profile's `$HOME` is set to `<profile-root>/home/`, and that subdirectory is **deliberately empty by design** (it's just where relative-path commands like `cd` land). All the real state lives at the profile root. The natural mental model is "if $HOME looks empty, the profile is empty" — but that's the wrong directory. This bites hardest when debugging a brand-new or suspect profile from the outside, because every shell command an operator runs from `$HOME` finds nothing.

**Real state location:**

```
~/.hermes/profiles/<name>/
├── state.db, state.db-wal, state.db-shm   ← session SQLite + WAL
├── sessions/, plans/, workspace/          ← domain dirs (often empty)
├── memories/, skills/, cron/              ← profile-local config
├── logs/agent.log                         ← all run history, session-tagged
├── .hermes_history                        ← raw user prompts + timestamps
├── SOUL.md, config.yaml, .env             ← persona + config
├── pairing/                               ← cross-device sync (usually empty)
└── home/                                  ← $HOME — EMPTY BY DESIGN
```

**Why `session_search()` may return 0 even when sessions exist:** The session DB uses SQLite WAL mode. While a session is in progress, FTS5 entries live in the WAL file, not in the main DB. If WAL hasn't been checkpointed (graceful shutdown didn't happen, or the session is still running), `session_search` reads only the main DB and reports zero hits. **False negative.** Always cross-check before declaring "no sessions."

**Diagnostic recipe (when `session_search` lies):**

```bash
PROFILE=~/.hermes/profiles/<name>
echo "=== unique session_ids in agent.log ==="
grep -oE '\[[0-9]{8}_[0-9]{6}_[a-f0-9]+\]' "$PROFILE/logs/agent.log" | sort -u

echo "=== most recent user prompts ==="
tail -20 "$PROFILE/.hermes_history"

echo "=== domain dir state (empty = unused profile) ==="
for d in sessions plans workspace memories cron/output pairing; do
  count=$(ls "$PROFILE/$d" 2>/dev/null | wc -l)
  echo "  $d: $count entries"
done

echo "=== WAL size (1.5MB+ while session active is normal) ==="
ls -la "$PROFILE/state.db" "$PROFILE/state.db-wal"
```
...
|---|---|---|
| `home/` empty | Normal — check profile root instead | Run the recipe above |
| `session_search` returns 0 | WAL not checkpointed | Don't trust it; use the recipe |
| sessions/plans/workspace all empty | Brand-new profile, never used | Confirm with `.hermes_history` + `logs/agent.log` |
| `.hermes_history` only has prompts from today | Profile is brand-new (or was reset) | No prior task exists; ask user for context |
| Logs are still growing | Session is still running | Wait for graceful shutdown; WAL will checkpoint |

**Heuristic to know when a profile is "really fresh" vs. "looks fresh":**
- **Really fresh:** `.hermes_history` ≤ today's prompts, `logs/agent.log` has no entries before today, all domain dirs empty, WAL is small or non-existent.
- **Has history but session_search says 0:** `.hermes_history` has prior days' prompts, `logs/agent.log` has older session_ids, WAL is large. False negative — session_search is lying; read the files directly.
...
| P10 (home/ empty) | `session_search` returns 0, `ls $HOME` empty | Run the P10 recipe: `.hermes_history` + `logs/agent.log` grep; don't trust FTS5 alone |
