...
### Profile's `home/` is empty — real state lives at the profile root

When you `ls ~/` from inside a running profile, you see an empty directory. That's not a bug — `$HOME` is set to `~/.hermes/profiles/<name>/home/` deliberately, and that subdirectory is empty by design (it's just where relative-path commands like `cd` land). All real state is at the profile root:

- `state.db` (+ `-wal`/`-shm`) — session SQLite DB
- `sessions/`, `plans/`, `workspace/`, `memories/`, `skills/`, `cron/` — domain dirs
- `logs/agent.log` — every run tagged with `[YYYYMMDD_HHMMSS_xxxxxx]`
- `.hermes_history` — raw user prompts with timestamps
- `SOUL.md`, `config.yaml`, `.env` — persona and config

When debugging "did this profile ever do anything?" or "where did the last task leave off?", **don't trust `session_search()` alone** — its FTS5 index lives in the WAL file, which may not be checkpointed if the session is still running or didn't shut down gracefully. Cross-check with `.hermes_history` (always present, always current) and `grep -oE '\[\d{8}_\d{6}_[a-f0-9]+\]' logs/agent.log | sort -u` (authoritative session_id list). Full reproduction + diagnostic recipe in `references/pitfalls-detailed.md` P10.
