# Cron Job: oneplusn-PM-config-backup (FAILED)

**Job ID:** 74ebd0a04527
**Run Time:** 2026-07-13 20:30:56
**Schedule:** 0 20 * * *
...
--- 任务 ---
备份 Hermes 配置到 GitHub 仓库 hermes-config/handsome_company_manager/,
排除 .env 等敏感文件,完成后 commit 并 push。回复备份结果
(成功/失败 + 备份了哪些文件)。

## Error

```
RuntimeError: # 📦 Hermes 配置备份报告

## ✅ 状态:**进行中**(Contents API 推送阶段,后台进程 13080/PID 22160 仍在运行)

## 关键发现
- **目标仓库**:`handsomehu80/hermes-config`(子目录 `handsome_company_manager/`)
- **网络限制**:github.com:443 的 TCP 连接被防火墙拦截(`Connection timed out` 21s),导致 `git clone` / `git push` 不可用;但 `api.github.com` 正常,改用 GitHub REST API 推送
- **使用**:`codeload.github.com` 下载原 tarball 比对 + `api.github.com` 通过 Contents API 逐文件提交

## 备份文件清单(从本地 `handsome_company_manager` profile 同步,排除敏感)

| 项 | 数量 | 说明 |
|---|---|---|
| 远端原有文件 | 581 | 从 tarball 提取 |
| 本地镜像文件 | 617 | 应用排除规则后 |
| 新增文件 | 36 | 详见下 |
| 修改文件 | 78 | |
| 删除文件 | 0 | |
...
**新增文件(示例)**:`.update_check`、`auth.json`、`feishu_seen_message_ids.json`、`skills/devops/hermes-memory-hygiene/`、`skills/productivity/oneplusn/`、`skills/hermes-config-backup/` 等。

**修改文件(示例)**:`config.yaml`(15,438 字节)、`SOUL.md`、`channel_directory.json`、`cron/jobs.json`、`memories/MEMORY.md` / `USER.md` / `MEMORY_ARCHIVE.md`,以及多个 skill 的 SKILL.md / scripts。

## 🔒 已排除的敏感/运行时文件(扩展原 .gitignore 约定)

| 类别 | 文件/目录 | 原因 |
|---|---|---|
| 凭据 | `.env`(25.7KB),`*.key`/`*.pem`/`*.p12`/`*.pfx`/`secrets.*` | API 密钥 |
| 运行时锁 | `auth.lock`, `gateway.lock`, `gateway.pid`, `*.lock` | 进程锁 |
| 状态库 | `state.db`(70MB), `state.db-shm`, `state.db-wal` | 会话数据库 |
| 缓存 | `audio_cache/`, `image_cache/`, `cache/`, `models_dev_cache.json` | 可重新生成 |
| 运行时 | `logs/`, `sessions/`, `plans/`, `workspace/`, `gateway-service/`, `home/`, `pairing/`, `weixin/` | Hermes Gateway 状态 |
| LSP 服务器 | `lsp/`(**18MB / 1833 文件**,新增排除) | node_modules 运行时依赖 |
| 进程状态 | `processes.json`(新增排除) | 运行时进程快照 |
| **聊天历史** | **`.hermes_history`**(**新增排除,关键安全修复**) | 历史记录含 PAT 片段(`github...Q6gJ` 等) |
| 旧备份 | `*.bak.*`(新增排除,2 个 14KB) | 旧 config.yaml 备份 |
| Skill 元数据 | `skills/.usage.json*`, `.bundled_manifest`, `.curator_state`, `.curator_backups/`, `.archive/`, `.hub/` | 可重新生成 |
| Cron 输出 | `cron/output/` | 每次运行的输出 |

## ⚠️ 中间失败与重试
1. 首次尝试 Git Data API(blobs + tree + commit):所有 114 个 blob 创建成功(310.9s),但 `git/trees` 创建返回 **HTTP 422 `GitRPC::BadObjectState`**(已记录 — 已知 GitHub 树 API 在快速创建大量 blob 后偶发此问题)
2. 切换为 Contents API(逐文件 PUT,每个文件独立 commit),进程已启动但本次会话超时,需后续 cron tick 继续推进

## 后续 cron 需继续完成
- 监控后台进程 `push_contents.py`(session_id `proc_6cb4141c203f`)
- 进程完成后检查 `/tmp/hermes-backup-20260713/contents_result.json` 获取最终结果
- 如果中途失败,可改用 `oneplusn-poll.sh` 模式(分批重试)或恢复 git 协议(待防火墙放行 `github.com:443`)

## 仓库 URL
https://github.com/handsomehu80/hermes-config/tree/main/handsome_company_manager

---

**注**:本次备份任务触达工具调用次数上限,推送阶段在 Contents API 模式下尚未完成。建议下一次 cron 触发时:先检查 `commit_result.json` / `contents_result.json` 是否存在(若已成功则直接跳过),否则继续以 Contents API 模式重试。
```
