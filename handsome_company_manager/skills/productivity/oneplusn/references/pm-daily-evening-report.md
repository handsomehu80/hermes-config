...
33. **Coordinate the daily §7 with the bihourly escalation state — DO NOT re-offer A/B/C after the bihourly has already shipped a one-liner (learned 2026-07-25).** The bihourly playbook has its own escalation ladder (`references/pm-bi-hourly-status-report.md` §2.10): 2 consecutive 2h reports = recommended PM-direct-action, 3+ consecutive = REQUIRED one-liner (no more A/B/C). The daily's own ladder is #23 above: 3 consecutive daily reports of 摸鱼 → PM-direct-action menu A/B/C. **These two ladders can collide**: when the boss is in a long deadlock, by the time today's daily fires, the boss has already seen 12+ bihourly reports — many of them already one-liners — AND yesterday's daily offered A/B/C. If today's daily also offers A/B/C, the boss sees the same question three times in three formats across 24h. **Fix (apply before writing §7):**
    ...
    1. Find the most recent bi-hourly report's `## Response` tail. Use friendly-name match per `references/cron-output-dir-picker.md` Option 1 (NOT file-count max — task-polling dir has ~5× more files and will mislead).
    2. Detect whether the most recent bihourly already emitted a PM-direct-action one-liner. Heuristic: the bihourly's tail contains both `gh pr merge` and the literal string `PM-direct-action` (or `一键合并` / `1 keystroke`).
    3. If yes → today's §7 must (a) cite the bihourly's #N reference, (b) **re-run `gh pr view --json mergeable,mergeStateStatus` for each PR in the bihourly's one-liner** (per SKILL.md "PM-direct-action one-liner staleness" caveat from 2026-07-19), (c) build a Δ table showing each PR's `mergeable`/`mergeStateStatus` NOW vs what the bihourly reported, (d) emit the same one-liner verbatim if Δ=0, or re-emit a corrected one-liner with rebase steps added/removed if any PR flipped state. **DO NOT offer A/B/C menu** — the boss has had 12+ chances; offering more choices is the wrong move.
    4. If no (bihourly hasn't yet escalated to one-liner) → use #23's A/B/C table format.

    **Real case (2026-07-25 daily run):** bihourly #131 already shipped the `gh pr merge 13 --merge` suggestion at 14:02 CST; yesterday's daily offered A/B/C. Today's daily anchored on the bihourly #131 one-liner + re-queried `gh pr view 13/14/15 --json mergeable` for the Δ table. Skipping the A/B/C menu and going directly to a 1-keystroke one-liner (with rebase steps for the dirty PRs) was the correct escalation. Verified §3 verdict: R/Y/G = 3/1/2 (better than yesterday's 4/2/1 because the daily escalation actually cleared the "boss non-response" row from yesterday).
