...
**Fallback when `git fetch origin` fails (learned 2026-07-21, PM #81).** The §2.9 `git merge-tree` pre-flight check requires `git fetch` to refresh `origin/main` locally. On the PM cron host, `git fetch origin` sometimes times out (`Failed to connect to github.com port 443 after 21s`); on 2026-07-21 this happened on PM #81. When `git fetch` fails:
