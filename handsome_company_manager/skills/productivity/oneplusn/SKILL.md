...
For **PM bi-hourly cron liveness** specifically — oneplusn employees have 6 dirs per profile due to the duplicate-registration trap (§"Duplicate cron registration"), not 3 — use `scripts/check_pm_cron_liveness.py --profile <name> --window-hours 2 --task-polling-only`. It walks task-polling output dirs, classifies transport by file size (<1KB wrapper marker / ≥5KB LLM envelope), detects UPPERCASE shadow duplicates (including reviewer alias `REVIEW → rev`), and then parses the payload after the **last** `## Response` marker to count semantic `[SILENT]` vs non-silent outcomes. **Do not equate a 60–100KB file with substantive work**: the repeated prompt dominates file size. Size proves liveness only; use response class + GH-side artifacts to distinguish healthy idle, stale-claim deadlock, and true cron-thrashing. Do **not** run an all-job 2h verdict: daily backup/cleanup jobs correctly have no output in most 2h windows and would create a false red. Use `--window-hours 26` without the filter for a separate full-profile audit. Pair with `references/pm-bi-hourly-status-report.md` §2.12 and `references/pm-daily-evening-report.md` §5 #24 for details.
...
13. **Duplicate cron registration — uppercase `REVIEW-*` vs lowercase `rev-*` jobs coexist for the same employee** (learned 2026-07-15, on the bi-hourly PM report run). When `oneplusn` registers cron jobs via `hermes cron add`, you may end up with **two registrations per employee per job type**: e.g. both `oneplusn-rev-task-polling` and `oneplusn-REVIEW-task-polling` exist in `<profile_home>/cron/jobs.json`, same prompt, same schedule, same offset minutes — but:
    - lowercase `rev-*` / `dev-*`: `script: None, no_agent: false` → runs the prompt directly → **real LLM worker** (`last_status=ok`)
    - uppercase shadow: `script` + `no_agent: true` → does **no real LLM work**, but its terminal status is not stable across wrappers. It may fail with exit 1 (`last_status=error`, seen on `REVIEW-*`) **or exit 0 with empty output** (`last_status=ok`, seen on `DEV-task-polling` with `dev_poll.py`). Both variants emit sub-1KB marker files and remain duplicates.
    
    Symptom: `cron/output/<uppercase-job-id>/` is full of `script failed` **or `silent (empty output)`** marker files; `jobs.json` shows two jobs per type with the same name mod case. The LLM can still be working normally because the lowercase job does the real work. **Never classify a worker from `last_status` alone**: case-normalize the friendly name, inspect `script`/`no_agent`, and count output-file size. Real LLM runs are typically 15-100 KB; shadow markers are about 150-200 bytes. Detection — run `scripts/check_pm_cron_liveness.py --profile <name> --window-hours 24 --task-polling-only` (or the deeper `references/cron-health-audit.md` recipe). If an uppercase job has only markers, remove it even when `last_status=ok`.
...
16. **PM bi-hourly report-build end-to-end recipe confirmed in production (PM #177, 2026-07-29)**. The bi-hourly fire at `04:00 UTC` (= `12:00 CST` filename) follows a complete recipe that combines:
    - **N-extraction**: 3-step fallback chain over latest bihourly file (canonical title → no-emoji title → footer "第 N 期")
    - **Identity probe**: `GET /user` → assert `login == Handsome-Manager` BEFORE any state query (catches wrong-account auth drift before any false reports)
    - **Parallel data pull via urllib.request** (NOT `gh` CLI): `/repos/<org>/<repo>/issues?state=all` (filter `[].pull_request == null`), `/repos/<org>/<repo>/pulls?state=all`, `/repos/<org>/<repo>/commits?since=<cutoff>`, `/repos/<org>/<repo>/events?per_page=30` (events API often omits `closed` events — use issue-resource `closed_by.login` instead)
    - **Cron-liveness walk across all 3 profiles**: for each `<profile_home>/cron/jobs.json`, iterate `j['jobs']`, resolve output dir `<profile_home>/cron/output/<j['id']>/`, classify latest file via the recipe below. The 3-issue pattern from #13 (uppercase shadows + lowercase real LLM) is now reliably detectable in any bihourly cycle.
    - **Mergeable re-query** (per §5 #24): for every PR named in a previous PM-direct-action one-liner, query `GET /repos/<org>/<repo>/pulls/<N>` fresh and emit a Δ table: `previous_verdict → current_verdict → SAME | FLIPPED`. If state flipped, re-emit the one-liner with rebase steps added/removed; if same, keep same one-liner text.
    - **Compose + write to disk + emit final response**: write to `<profile_home>/cron/output/d26c66fbbdd0/<CST-filename>.md` (CST filename, UTC mtime) before emitting. Final response IS the report body (cron system auto-delivers to Feishu home channel — no `send_message` needed).
    
    Full recipe in `references/pm-bi-hourly-status-report.md`. **Confirmed working in PM #177 with 481 CJK chars / 3.5 KB report size; 0 GH-side artifact drift (the report itself isn't a drift dump; this cron type is exempt per the Exclusion list #3 because the file ends with the canonical `# 📊 PM 双小时状态报告 #N` title + §0–§6 sections).** When a cron prompt says "这是第 N 期(从当前时间计数)" without specifying N, derive N from the previous output: list files in `<profile_home>/cron/output/<job_id>/` sorted by mtime, read the latest, and extract N. Numbering is monotonically increasing (e.g. 12 fires/day ≈ 111 by day 11). If the regex fails or the file is missing, fall back to `len(files)+1` as a rough lower bound and flag "Δ unverified" in the report header.

    **The tightest regex pattern (refined 2026-07-27):** require the trailing `(` of the `(YYYY-MM-DD HH:MM UTC)` date format that real report titles always carry. Skill-content pollution from the loaded SKILL.md appears as bare backticked references like `` `#135` report in tail `` — no `(` follows — so the anchor filters them out cleanly without needing the "take last match" workaround. Use this:

    ```python
    from pathlib import Path
    import re
    bihourly = Path("C:/Users/Administrator/AppData/Local/hermes/profiles/handsome_company_manager/cron/output/d26c66fbbdd0")
    latest = sorted(bihourly.glob("*.md"), key=lambda x: x.stat().st_mtime, reverse=True)[0]
    txt = latest.read_text(encoding="utf-8", errors="replace")
    m = re.search(r'#\s*📊\s*PM\s+双小时状态报告\s*#\s*(\d+)\s*\(', txt)
    prev_N = int(m.group(1)) if m else None
    this_N = prev_N + 1 if prev_N else 1
    ```

    Real case (2026-07-27 PM #153): the previous bi-hourly file had TWO candidates — pos=39802 `` `#135` `` (skill-content pollution) and pos=79957 `#152(2026-07-27 01:48 UTC)` (real title). The tightened regex returned `#152` on the first match. The bare `双小时状态报告 #(\d+)` pattern (without the emoji + `\s*\(`) would have returned `#135`. **Always pair with the trailing "完成" line as a sanity guard** — see `references/pm-bi-hourly-status-report.md` §5 #22 for the full multi-source corroboration recipe and the cron-wrapper line-0 trap.

    **Title-format drift fallback (learned 2026-07-29, PM #174)**: the canonical title `# 📊 PM 双小时状态报告 #N(...)` is not always followed by the LLM. Observed N=173 title was `状态报告 #173 (2026-07-28 20:00 UTC)` (no `📊 PM` prefix), and the primary regex missed it. **Use a fallback chain**, not a single pattern:
    ...
    # Primary: canonical title (with 📊 PM prefix and trailing `(` for date)
    m = re.search(r'#\s*📊\s*PM\s+双小时状态报告\s*#\s*(\d+)\s*\(', txt)
    # Fallback 1: alternative title (no 📊 PM prefix, observed N=173)
    if not m:
        m = re.search(r'#\s*状态报告\s*#\s*(\d+)\s*\(', txt)
    # Fallback 2: footer "第 N 期" pattern (last-resort only; verify the file is a real report first)
    if not m:
        m = re.search(r'第\s*(\d+)\s*期', txt)

    prev_N = int(m.group(1)) if m else None
    this_N = prev_N + 1 if prev_N else 1
    # Sanity: if both Fallback 2 fires AND the file is a drift dump (size>50KB + skill headings),
    # trust the file-count heuristic instead (len(sorted .md files) + 1) and flag "Δ unverified".
    ```

    The footer pattern (`第 N 期`) is a real signal IF and only IF the file has been classified as a real report (size < 50KB OR has `# 📊 PM 双小时状态报告` OR has `双小时状态报告` heading + `## 1.`/`## 3.`/`## 5.` sections). Otherwise it's skill-content pollution and must be ignored. The chain order (primary → fallback1 → fallback2) is intentional: each subsequent pattern is progressively less specific, so use the highest-priority match.
...
20. **`datetime.utcnow()` returns local time, NOT UTC, on Windows** (learned 2026-07-28, PM #167 bi-hourly run). The naive `datetime.datetime.utcnow()` is a notorious Python API: it stamps the value as UTC but returns the **local clock reading**, which on a CST (UTC+8) Windows host is 8 hours ahead of true UTC. The `int(.timestamp())` form also lies (it treats the naive datetime as local). Symptom: comparing `now().timestamp()` to file `mtime` gives an "8 hours in the future" or "8 hours in the past" mismatch — the cron LLM reads file `mtime` as UTC, but `utcnow()` says it's UTC-now-8h, which doesn't reconcile. **Fix**: always use `datetime.datetime.now(datetime.timezone.utc)` for UTC, and pair it with `.timestamp()` (which respects the tz) when comparing to file mtime (mtime is always epoch-UTC). Canonical pattern:
    ...
    This bites the cron LLM whenever it tries to compute "files in last N hours" from filenames (because filenames are CST) vs from mtime (because mtime is UTC) — pick one and stick to it. **The cleanest invariant**: derive "now" and "cutoff" both via `now(tz=utc)`, and filter by `file.stat().st_mtime >= cutoff.timestamp()` (always UTC).

21. **`git merge-tree` for a PR fails with `fatal: ambiguous argument '<branch>': unknown revision` when the head branch isn't in the local worktree** (learned 2026-07-28, PM #169 bi-hourly run, on PR #13). The §2.9 `git merge-tree` pre-flight check assumes `git fetch origin` has populated every PR's head branch locally — but PR head branches (e.g. `feat/issue-10-ralph-loop-poc`) may be missing from local refs even after a successful `git fetch origin` if (a) the branch was created via GitHub UI without ever being on a local clone, (b) the local repo is shallow, or (c) upstream refspec restrictions filter PR refs. Symptom: `git merge-tree $(git merge-base origin/main origin/<branch>) origin/main origin/<branch>` errors with `fatal: ambiguous argument 'feat/issue-10-ralph-loop-poc': unknown revision or path not in the working tree` — same shape as if the branch were never pushed. **Fix order** (try each before falling back): (1) `git fetch origin <branch>` — if it succeeds, retry merge-tree; (2) if fetch errors with "couldn't find remote ref", the branch really doesn't exist on remote (rare — branch was deleted, or it's a fork PR); (3) if merge-tree still fails after fetch, mark the PR in the Δ table as `⚠️ branch-not-local — verdict from previous report (re-verify PR state via gh pr view --json headRefName,state,additions,merged)` and DON'T include it in the deterministic mergeable pre-flight. **Decision order refinement** (supersedes §2.9 when this gap hits): (a) local `git merge-tree` works → use it; (b) fails because branch not local → targeted `git fetch origin <branch>` + retry; (c) fails again → fall back to previous report's verdict + current `gh pr view` state; (d) `gh pr view --json mergeable` returns None → "GitHub not yet computed" (wait for next cycle). **Do NOT silently fall back to a stale mergeable verdict** — always re-verify PR is still OPEN with same head via `gh pr view --json headRefName,state,additions,merged` before carrying forward. Real case 2026-07-28: PR #13 head `feat/issue-10-ralph-loop-poc` was missing from local after `git fetch origin` succeeded; the report carried forward N=168's `MERGEABLE/CLEAN` verdict (the N=167→N=168 transition had also reported the same, so two consecutive reports agreed).
...
**Prevention at deploy time:** when running `oneplusn init --work-dir <team>`, validate the path BEFORE registering crons. Directory existence is necessary but **not sufficient**: an empty or non-repository directory makes `git log` fail while GitHub polling and cron liveness still look green.

For code-oriented employees and PM reports that use local commit evidence, require all three checks:

```bash
python -c "import os; assert os.path.isdir(r'<work-dir>')"
git -C "<work-dir>" rev-parse --is-inside-work-tree
git -C "<work-dir>" remote get-url origin
```

If the user intentionally wants a profile-home-only worker, drop `workdir` from the cron instead of pointing it at an empty placeholder directory. If a **read-only status report** discovers that the local Git check fails, do not translate the failed `git log` into “0 commits.” Report “local commit evidence unavailable,” then use the GitHub commits API for the same UTC cutoff as a bounded fallback:

```bash
gh api 'repos/<org>/<repo>/commits?since=<ISO-UTC-cutoff>&per_page=100' \
  --jq '.[] | [.sha[0:7],.commit.author.date,.author.login,.commit.message] | @tsv'
```

Keep the two facts separate in the report: remote activity may be zero while the local execution/audit base is still misconfigured. If the user wants a OneDrive or cloud-mirrored path, give them a choice: verified local repository → use it; symbolic/non-repo path → drop `workdir` and let jobs default to profile home. The deployment checklist should ask both “does the workdir exist?” and “is it the expected Git repository?” — see `references/deployment-checklist.md`.
...
1. Files in `<profile_home>/cron/output/<job-id>/` are unusually large — observed trajectory (PM task-polling dir, 2026-07-12 to 2026-07-29): **15 KB → 40 KB → 60 KB → 80 KB → 95 KB**, monotonically growing ~5 KB/day as the prompt inlines the full SKILL.md and SKILL.md grows with each new pitfall. A real response is ~5–15 KB. **Treat file size as a warning sign, not a definition** — the [SILENT]-tail discriminator (rule #3 below) is authoritative.
...
**Re-baseline cadence:** the "60–80 KB" threshold documented on 2026-07-25 became stale within a week — re-query the actual file size in the cron output dir on the first run of each month and update the threshold in this rule. The 2026-07-29 measurement was **94–96 KB** for PM task-polling (cef7e567). For reference, PM bi-hourly dir (d26c66fbbdd0) holds at **100–105 KB** (SKILL.md dump + 6-section report body) and the canonical `# 📊 PM 双小时状态报告 #N` title + `## 0`–`## 6` section structure confirms it's a real report per the Exclusion list, not drift.
...
**Exclusion (3 cron types now legitimately trigger the size + heading signals — confirmed 2026-07-27, PM task-polling tick at 15:16 UTC, 5 consecutive 82225-byte output files):**

1. **daily-evening-report** — verified 2026-07-26 (PM #136). Look for `# 📅 PM 项目日报` + `## 0`–`## 8` in tail.
2. **bihourly** — same date. Look for `# 📊 PM 双小时状态报告 #N` + `## 0`–`## 6` in tail.
3. **task-polling when the final response after `## Response` is exactly `[SILENT]` (or `[SILENT\n` with missing closing bracket)** — verified 2026-07-27, 5 consecutive task-polling files (15:16 / 14:47 / 14:16 / 13:46 / 13:17 UTC) all 82225 B, all dumped the full oneplusn SKILL.md verbatim, all ended with `[SILENT]` at the tail. The task-polling prompt DOES inline the full oneplusn SKILL.md in the current deployment (the file body opens with `[IMPORTANT: The user has invoked the "oneplusn" skill, indicating they want you to follow its instructions. The full skill content is loaded below.]`) — the earlier claim that task-polling does not load the SKILL.md is **outdated** for this deployment. The `[SILENT]` protocol section is the first actionable guidance in the loaded skill body, so a correct LLM reading the prompt + skill produces `[SILENT]` as the final answer when 0 issues are assigned to the persona. `[SILENT]` at the tail is the correct decision despite the surrounding skill-content dump. **Only flag as drift when the tail contains something OTHER than bare `[SILENT]`** — a verbose "investigation" report, attempted `gh` calls that failed, or a partial report is a real drift signal (the LLM got the right SKILL.md context but failed to apply the silent-exit protocol). **Lenient tail check** (PM #186, dev `42173ac76d3f` outputs): the LLM sometimes drops the closing `]` on token boundaries, producing `[SILENT\n` instead of `[SILENT]\n` — semantically identical silent-exit, do NOT flag as drift. Use `re.search(r"\[SILENT\]?\s*$", tail.rstrip())` instead of exact string equality. See `references/pm-bi-hourly-data-quality.md` DQ-2 for the full discriminator recipe.

For all 3 exclusions, the discriminator is the content after the LAST `## Response` marker. If the matching structure is present (report headers + sections for #1/#2, bare `[SILENT]` for #3), the file is NOT drift; the LLM executed correctly and the skill content is just sitting in the prompt context.

**config-backup and memory-cleanup still get the strict interpretation** — verified separately (2026-07-27 sample: latest config-backup `2026-07-26_20-02-33.md` = 82272 B with NO skill headings present, just a `[SILENT]`). Their prompts do not load the full SKILL.md, so skill headings in their output would be an unambiguous drift signal. (If a future deployment starts inlining the SKILL.md into those prompts too, add them to the exclusion list with the same `[SILENT]` discriminator as task-polling.)
...
- **0-activity 1 期 ≠ 摸鱼;verify consecutive zeros before flagging 🔴.** The bi-hourly status rule says "连续 2 期 0 活动 = 🔴 摸鱼嫌疑". PM must NOT just trust the current 2h window — read the previous N reports from `<profile_home>/cron/output/<job_id>/*.md` to count consecutive zero-activity windows. 0 in one window is normal (employee may be waiting on cron, mid-task, or simply not yet due). Only after confirming 2+ consecutive zeros should PM escalate. **Canonical re-runnable script**: `python scripts/count_consecutive_zero_activity.py --window 6` — reads the most recent 6 bihourly reports, extracts the §3 每人贡献 table from each, and counts per-role trailing 0-activity runs. Returns JSON with `{trailing_zero_run, entries[]}`; exit 1 if any role has trailing_run ≥ 2. Paste the verdict line into §3's 摸鱼信号 column (e.g. "🔴 stale-claim 5+ consecutive (run=N, fix: unblock gating PRs)"). Distinguish **真摸鱼** (cron firing but employee idle, no follow-up after dispatch) from **deadlock** (mutual waiting — see next pitfall) — they look identical from above but require different fixes. The script only counts; classification (摸鱼 vs stale-claim vs healthy-idle) is still PM's job via §5 #23 / §5 #26.
...
- **Boss-merge-PR deadlock — distinct from AND-trigger (added 2026-07-18).** AND-trigger deadlock has both sides owned by employees who can be派单. **Boss-merge-PR deadlock** is structurally similar (3+ windows of 0 GH activity, cron firing, LLM `[SILENT]`) but the unblock action is on the **boss**, not on an employee: reviewer wrote `Verified and closed (PASS, PR #N 可合并)` on the Issue → Issue CLOSED → but PR itself stays `state=OPEN` because RULES say reviewer validates, not merges. Downstream Issues / dev follow-up branches block on PR merge. PM's diagnostic is in `references/pm-bi-hourly-status-report.md` §2.7 (the 4th state of the cron-liveness classification: cron firing ✅, LLM `[SILENT]` ✅, open assigned Issues with "等 PR 合并" condition ✅, ≥1 OPEN PR > 24h with reviewer PASS ✅). PM's fix after 2 consecutive 2h reports showing this state: stop asking boss A/B/C, **draft a `gh pr merge 14 15 13` ready-to-paste line + brief merge-readiness summary** and include it in the next report. Frame it as "1 keystroke unblocks everything" — boss has nothing left to decide. Real case (2026-07-18): PR #14 (+435), PR #15 (+901), PR #13 (+917) all OPEN > 5 days; dev's #19/#20 follow-up branches sat idle 57h; boss had not picked A/B/C from 2 prior reports. Lesson: when the previous A/B/C menu has been ignored for ≥2 consecutive reports and the deadlock is boss-action-shaped (not employee-shaped), PM-direct-action with a copy-paste one-liner is the correct escalation — A/B/C menus stop being useful past that point.
   ...
   - **PM-direct-action one-liner staleness — re-verify mergeable JUST BEFORE execution** (learned 2026-07-19, PM #52 on boss-merge-PR deadlock day 73h). The previous report's `merge-tree`-based verdict is NOT safe to copy-paste into a new one-liner, because PR mergeable state can flip between reports in EITHER direction: (a) upstream merges into `main` invalidate previously-clean PRs (real case: PR #15 was reported "clean" by PM #51's `merge-tree` check, but came back `CONFLICTING` per `gh pr view --json mergeable` 2h later in PM #52 — the cause was PR #18 Snake merging into `main` on 7/14 between reports, which local `git merge-tree` missed because the Windows host couldn't reach `github.com:443` for `git fetch`); (b) silent force-pushes to the head branch by the dev/reviewer can also flip state either way. **Fix**: every PM-direct-action report (especially consecutive reports against the same deadlock) MUST include a "Δ vs previous one-liner" table showing current `gh pr view --json mergeable` for each PR. If any PR's state changed (CLEAN↔CONFLICTING), the one-liner MUST be re-emitted with rebase steps added or removed. **Authoritative source** is `gh pr view --json mergeable` (GitHub-side, freshly queried) — NOT local `git merge-tree`, which can lie in either direction when `origin/main` is unreachable, stale, or when the head branch has unpushed commits. When in doubt, query `gh pr view --json mergeable,mergeStateStatus,headRefName` for all PRs in one shot and diff against the previous report's table before emitting the one-liner. See `references/pm-bi-hourly-status-report.md` §2.10 for the updated one-liner template with the state-diff row.

      - **PM-direct-action escalation fatigue — fold §5 progressively after sustained boss non-response** (learned 2026-07-31, PM #203 on boss-merge-PR deadlock day 18+). When the §2.10 escalation threshold has been tripped and the boss STILL hasn't acted across multiple consecutive 2h reports, the one-liner section itself becomes fatigue-creating content: re-emitting the same 200-字 `gh pr merge 13 --merge` block every 2h trains the boss to ignore §5 and wastes the report's character budget. **Progressive compression cadence** (use these thresholds to shape §5 size each cycle, NOT to abandon the escalation — always keep the one-liner visible, just shorter): N=1-2 reports after deadlock detected → full one-liner (~250 字) with Δ table and merge-readiness summary (current behavior); N=3 reports → condensed (~120 字) with one-liner + 1-line Δ verdict; **N=4+ reports → single-line reminder** (≤40 字: the bash command + current mergeable state + last-action timestamp); N=8+ reports → single-character reminder `→ 合 #13` (only safe if prior 4 reports have been folded without boss engagement). The PM MUST announce the impending fold IN the N=2 or N=3 report's §7 (e.g., "下次 N+1 若仍未动,我将折叠 §5 为单行") so the boss sees the fold coming and isn't surprised by the format change. **Re-allocation rule**: when §5 shrinks, the freed character budget moves to §2 (new risk rows for any silent drift) and §7 PM 洞察 (analysis of why the silence might be intentional: "老板已读 X 期 / 切换到其他项目 / 决定暂时搁置 — 都不是 PM 主动该打破的局面"). **The "silence-as-choice" reframing**: after 5+ consecutive reports with no GH activity AND no boss response, the situation is no longer "stuck" — it is "boss has been informed and is choosing not to act". PM should explicitly call this out in §7 to anchor the boss's choice ("沉默进入常态化 — 你不再需要 A/B/C,只需要'按回车'或'明确叫停'"), and §5 should drop the words "urgent" / "紧急" / "请立刻". **Do NOT** ever stop emitting §5 entirely or skip the report — the boss may still want to monitor and any future action needs the §1/§3 context for situational awareness. **Real case** (N=197 → N=203, 7 consecutive 2h reports): PM #202 self-promised in §7 to fold §5 if no action; PM #203 executed the fold to a single-line reminder (~30 字 vs ~250 字), dropping §5 from the report. The fold gave §2/§3 more room and signaled "I'm tracking, but I'm not nagging". Companion caution: `scripts/count_consecutive_zero_activity.py --window N` reports `trailing_run=0 [green] no trailing zeros` when §3 text in the latest report already shows "连续 16 期 0" — the script's classification logic does not match what the report text says; PM should treat the script's verdict as informational only and count zero-activity streaks manually from the §3 rows, OR patch the script to parse the trailing-run number from the §3 row text itself (e.g., regex `连续 (\d+) 期`). Fold-thresholds table is documented inline in this pitfall (above); `references/pm-bi-hourly-status-report.md` already exceeds the 100KB skill-content limit and cannot be further extended without restructuring.

   **Fallback when `git fetch origin` fails (learned 2026-07-21, PM #81).**
1. **Skip `git merge-tree` entirely** — it will operate on stale local `origin/main` and produce a wrong verdict.
2. **Sole authority = `gh pr view --json mergeable,mergeStateStatus`** — works because `gh` uses its own authenticated HTTPS path via `~/.config/gh/hosts.yml`, separate from git's fetch path. The `gh` CLI was still reachable when `git fetch` failed.
3. **Note in §5 PM-direct-action one-liner**: "git fetch unreachable this cycle — `gh pr view` verdict is sole authority". Boss should still paste-merge on `mergeable=true`; on `mergeable=UNKNOWN` (rare — only when GitHub hasn't computed yet), tag the PR as ⚠️ and recommend `gh pr view <N> --json mergeable` re-query right before the merge keystroke.
4. **Don't fail the report** — the §2.9 cross-check is enhancement, not gate. The §2.12 Δ table is enough; §2.9 is the belt-and-suspenders second source.
...
- [`references/pm-poll-gate.md`](references/pm-poll-gate.md) — deterministic task-polling preflight: persona identity → direct repository access → assigned Issue enumeration, with an explicit side-effect-free `[SILENT]` zero-queue branch.
...
- [`references/pm-bi-hourly-data-quality.md`](references/pm-bi-hourly-data-quality.md) — **PM bi-hourly data-accuracy pitfalls (DQ-1, DQ-2, DQ-3)**. Mandatory reads: PR `additions`/`deletions` return 0 from the list endpoint but real values from the detail endpoint (silent corruption if you only use list); the `[SILENT]` tail discriminator must accept the no-closing-bracket variant `[SILENT\n` (verified dev `42173ac76d3f` outputs, PM #186); and the urllib.request cross-check that underpins both. Load this alongside `pm-bi-hourly-status-report.md` on every bi-hourly cron fire.
- `references/pm-bi-hourly-status-report.md` — **PM's recurring 2h status report cadence** (distinct from task-polling and from PM Mode). **⚠ File-name caveat (verified 2026-08-05, PM #221): the file at this path is the **scheduler-cadence-gap pitfall** ("Ghost OK — Cron Jobs.json Lies About Liveness When Scheduler Is Broken", first observed PM #209), NOT the main recipe.** The main recipe content (Windows-safe data-collection commands, 6-section report template, 摸鱼信号 0/3+ rule, §0 5 numbers, §2.5 4-state cron-liveness classification, §2.7 boss-merge-PR deadlock detection + PM-direct-action escalation, §2.9 `git merge-tree` pre-flight, §2.10 PM-direct-action one-liner template with Δ table, §2.11 `--paginate > file` JSON-corruption trap, §2.8 cron-output-dir-to-friendly-name picker, §5 #23 stale-claim + [SILENT] loop, §5 #24 read-previous-report recipe, §5 #27 boss-close-without-merge partial intervention, **§5 #24's `max by file count` heuristic is INVERTED** — task-polling fires every 30min so it accumulates ~5x more files than the bihourly dir; use `references/cron-output-dir-picker.md` for the corrected size-filter + friendly-name-match heuristic) is **inline in the SKILL.md body above**, not in any reference file. The reference file (`pm-bi-hourly-status-report.md`) should be renamed to `pm-bi-hourly-cadence-gap.md` to match its content; until then, do NOT load it expecting the main recipe — load the SKILL.md body directly. **GH API field-name note (learned 2026-08-05, PM #221)**: PR detail endpoint returns the mergeable state under key `mergeable_state` (e.g. `"clean"` / `"dirty"` / `"blocked"` / `"unstable"` / `"behind"`), NOT `merge_state_status` — using the wrong key raises `KeyError`. Always pair with `mergeable` (bool) for the binary verdict; the mergeable_state string gives context (CLEAN / DIRTY / UNSTABLE / BEHIND).
- `references/pm-daily-evening-report.md` — **PM's once-per-day end-of-day report cadence** (cron schedule `0 15 * * *` interpreted in local CST — actual fire time is **15:00 CST = 07:00 UTC** on this Windows host, NOT 23:00 CST as the cron prompt header implies; see this file §5 #17 for the timezone correction + cross-day window rationale). 24h rolling window, ≤2500 字 budget, includes the "Δ vs yesterday" cross-day comparison pattern. Three operational patterns to internalize before writing §3: (a) `gh api .../issues` returns PRs too — must filter `select(.pull_request == null)`; (b) `last_status=None` (never fired) vs `last_status=error` (fired and failed) are different signals — see §2.3; (c) **§23 — after 3 consecutive days of 摸鱼, escalate §7 from "老板拍板" to a PM-direct-action menu (A/B/C with time/cost/limitations)** because yesterday's ignored decision points don't auto-resolve. Also §24 (response-aware cron classification: file size proves liveness only; parse semantic `[SILENT]` vs non-silent before calling cron-thrashing), §25 (open-PR age column in §2), §26 (boss non-response row in §3), §36 (deadline-aware liveness for calendar crons—raw file age before the next due time is not drift), and §37 (scope project commits by repository, verify config-backup SHAs in their real target repo, and handle nullable Commit API authors). Load this on every PM daily cron tick.
...
- `scripts/count_consecutive_zero_activity.py` — **PM bi-hourly 摸鱼 signal corroborator**. Reads the most recent N **distinct report periods** from the PM output dir, deduplicating body-only artifacts and cron-wrapper envelopes by extracted report number before parsing each report's §3 每人贡献 rows. This matters because one fire may create two files with the same N; counting files inserts a false “not in report” entry and resets the trailing-zero run. Run at the start of every 2h report build: `python scripts/count_consecutive_zero_activity.py --window 6`. Exit 0 = no role has trailing_run ≥ 2; exit 1 = at least one role has 2+ consecutive 0s (boss's hard 摸鱼 threshold). Pairs with `scripts/check_pm_cron_liveness.py` (cron liveness) and `references/pm-bi-hourly-status-report.md` §5 #23 (stale-claim classification) — the script counts but does NOT classify.
- `multi-profile-team`
