...
**Review-required handoff trap when the card has child tasks.** The `kanban_block(reason="review-required: ...")` pattern is correct for code review, but it has a sharp edge: if your card was created with `--parent` links from a follow-up card (e.g. qa is waiting on your eng work), the follow-up will not auto-promote when you `block` — only when you reach `done`. Two patterns to choose from, picked at card-creation time:

- **If you want downstream cards to start automatically after your work ships:** call `kanban_complete` and put the review-required marker + handoff metadata in a `kanban_comment` first. The card transitions to `done` and the dispatcher promotes the next card. Reviewer reads the comment thread to see the review-required flag.

- **If you want a hard review gate before downstream runs:** accept that the orchestrator (PM) must `hermes kanban unblock <your-id>` after a human/QA signs off. The unblock re-spawns you; you read the comment thread, see the verdict, and `kanban_complete`.

Don't mix the two patterns inside one workflow. Pick the one that matches the team's review culture and bake the choice into your profile's SOUL.md. Mixed patterns leave cards stuck in `blocked` with no one knowing who's responsible for the next click.
