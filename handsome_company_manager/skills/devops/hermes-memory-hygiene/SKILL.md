...
- **Cross-profile LLM-init differential is the strongest refutation of "permanent block"** (learned 2026-08-07, PM memory-cleanup cron run #19). The lock-mtime check above detects *simultaneous* daemon attempts but doesn't tell you whether a neighbor daemon actually reached LLM init. **Stronger diagnostic**: in the same cycle where your active profile's daemon times out, read the neighbor profile's latest log lines (the trailing 5–10 lines of `~/.hindsight/profiles/<neighbor>.log`). If the neighbor daemon reached `Connection verified: <provider>/<model>` (LLM init successful) and then ran for ~60s idle before closing, the active profile's failure is **intermittent**, not permanent — the cross-profile result rules out credential-missing, host-firewall, or permanent-block root causes. **Real case (2026-08-07 PM cycle #19)**: PM profile daemon timed out at 180s with HF Hub unauthenticated warning text re-surfacing in stderr at +1s after startup; `handsome_company_developer.log` (same cycle, mtime 0m) showed the dev daemon reached `Connection verified: minimax/MiniMax-M3` at 21:04:02, then ran 60s idle before closing. The differential (PM fails at HF gate, dev reaches LLM init same cycle) directly refuted the "permanent block" hypothesis that the 8-06 housekeeping had leaned toward. **Re-ranking implication**: when the cross-profile LLM-init differential is positive, option (a) `set HF_TOKEN=*** becomes the strongest unblock candidate, because the only thing separating the two profiles' outcomes this cycle is whether they hit HF rate-limit (intermittent, solved by authentication). Use this differential as an explicit decision input in the re-ranked (a/b/c) table — don't bury it in the diagnostic-implication sub-bullet. **Companion rule to the upstream-gate-drift pitfall**: the `PostgreSQLBackend is not initialized. Call initialize() first.` warning is itself a **misleading trace-write side-effect** — it appears whenever the LLM trace subsystem writes a record but the trace backend (PostgreSQL) wasn't initialized, regardless of where the daemon actually died. Treating it as the active gate on its own (without corroborating daemon-stderr evidence) over-attributes failure to the embedded-postgres init and demotes (a) incorrectly. Always cross-reference the Postgres warning with at least one of (i) HF warning text in daemon stderr (proves gate is HF download), (ii) cross-profile LLM-init differential (proves failure is intermittent), or (iii) `git fetch` failure / DNS resolution log (proves gate is network) before re-ranking. Without that cross-check, the "Failure GATE can drift upstream" pitfall can lead you to demote (a) for the wrong reason.
...
- [ ] **Cross-profile LLM-init differential check (when active daemon times out at HF gate)**: read the trailing 5–10 log lines of `~/.hindsight/profiles/<neighbor>.log` for any sibling profile. If a neighbor daemon reached `Connection verified: <provider>/<model>` (LLM init successful) in the SAME cycle, that's a strong positive signal the active profile's failure is intermittent, not permanent — record the differential in the housekeeping re-ranked (a/b/c) table as a direct input, not just an implication. Don't accept the `PostgreSQLBackend is not initialized` warning as evidence of the active gate without corroborating daemon-stderr or cross-profile evidence (it's a recurring trace-write side-effect, see §"Cross-profile LLM-init differential" pitfall).
