...
- **`auth.json` is sensitive** (contains `MINIMAX_CN_API_KEY` reference with `source: env:...` and other credential pool metadata). The 2026-07-13 backup committed it. If you see it in the remote, propose deletion in a separate PR — don't piggyback on a backup commit.
