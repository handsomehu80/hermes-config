...
9b. **With `MSYS_NO_PATHCONV=1` set, native Windows binaries launched from bash silently re-root `/tmp/...` to `C:/tmp/...`** (NEW 2026-08-16 PM config-backup). When the environment has `MSYS_NO_PATHCONV=1` (and/or `MSYS2_ARG_CONV_EXCL=*`), MSYS no longer translates POSIX paths in argv — so `git clone ... /tmp/hermes-backup/foo` passes the LITERAL string `/tmp/hermes-backup/foo` to native `git.exe`, which resolves it as **drive-relative** → `C:/tmp/hermes-backup/foo`. Meanwhile bash builtins (`ls`, `cd`, `mkdir`) still see `/tmp` as the MSYS mount `%LOCALAPPDATA%\Temp`. Result: clone "succeeds" (EXIT=0, "Cloning into..." printed) but `ls /tmp/hermes-backup/foo` says **No such file or directory** — the repo landed in `C:\tmp\` instead. Symptom fingerprint: `git clone` prints no error, `ls <dir>` fails immediately after, and `ls /c/tmp/...` shows the "missing" directory. **Fix: never pass `/tmp/...` POSIX paths to native binaries on this host — always use explicit `C:/Users/Administrator/AppData/Local/Temp/...` forward-slash Windows paths for clone targets, script args, and any path that crosses the bash→exe boundary.** This is the mirror-image of gotcha #9 (Python UNC trap): #9 hits Python `pathlib` reading POSIX paths; 9b hits native exes receiving POSIX paths. Diagnostic one-liner: `git -c ... clone <url> /tmp/x && ls /tmp/x` failing while `ls /c/tmp/x` succeeds = this trap. (Verified 2026-08-16: 3 "successful" clones landed in `C:/tmp/hermes-backup/`; switching to explicit `C:/...` paths fixed it. Also found a stale `C:/tmp/hermes-backup/hermes-config` clone from an earlier run with the same cause.) **Python `Path("/tmp/...")` is the same trap on Windows** — `pathlib` interprets a leading-slash POSIX path as a UNC `\\tmp\...` reference and raises `FileNotFoundError: '\\tmp\\...'` on read. Always use the resolved Windows-absolute form (`C:/Users/<user>/AppData/Local/Temp/hermes-backup/...`) in any `pathlib` / `execute_code` code, never the bash-side `/tmp/...` shortcut. Verified 2026-08-06 PM config-backup: `Path("/tmp/hermes-backup/orig-mgr-1786017908/...").read_text()` failed with `FileNotFoundError: [Errno 2] No such file or directory: '\\tmp\\hermes-backup\\...'`; switching to `C:/Users/Administrator/AppData/Local/Temp/hermes-backup/orig-mgr-1786017908/...` resolved it. The same applies to any leading-slash POSIX path on Windows (`/c/...`, `/tmp/...`, `/var/...`) — they all become UNC `\\<segment>\...` references.
