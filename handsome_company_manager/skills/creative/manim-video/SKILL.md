...
### Windows Quickstart (LaTeX optional)

`setup.sh` hard-fails on missing `pdflatex`, but **LaTeX is only required for `MathTex` and `Tex`**. For text/geometry/shape scenes (titles, motion graphics, infographics, festival animations, 3D scenes), skip LaTeX entirely:

```bash
# Minimum install on Windows
python -m pip install manim     # ffmpeg already needed; choco install ffmpeg or download
# Skip LaTeX — use only Text(), not MathTex()/Tex()
```

Verified workflow: `Text("端午安康", font_size=180, weight=BOLD)` + `Polygon`/`VGroup` + `FadeIn/Write/Transform` all render fine without LaTeX. Render at `-ql` for 5–15s draft previews, then `-qh` for production 1080p. Output lands at `media/videos/<scriptname>/<quality>/<SceneName>.mp4` relative to cwd.

### Web-viewable content → image fallback (no Pillow/cairosvg)

When you need an SVG (or any web-renderable file) rasterized on a host that lacks Pillow/cairosvg/svglib, the browser tool is the fastest path. See `references/web-content-to-image.md` for the full procedure and WeChat-delivery pitfalls.
